...
func TestMapper_mapPlayer(t *testing.T) {
	...
	infobasketDto := mock_data2.CreateMockPlayer(id, plusMnius, seconds, localFirstName, localLastName, enFirstName, enLastName, birthdate, isStart)
...
func TestMapper_mapTeam(t *testing.T) {
	...
	infobasketDto := mock_data2.CreateMockTeamBoxScoreDto(alias, name, score, teamId, playersCount)
...
func TestMapper_mapGame(t *testing.T) {
	...
	infobasketDto := mock_data2.CreateMockGameBoxScoreResponse(homeTeamId, awayTeamId, gameStatus, maxPeriods, scheduled.Format("02.01.2006"), scheduled.Format("15.04"))
