...
import (
	...
	mock_data2 "IMP/app/internal/statistics/infobasket/mock_data"
	"testing"
	"time"
)

// Setup a test Provider with mocked dependencies
func setupTestProvider(mockClient *infobasket2.MockClient) *Provider {
	return &Provider{
		client: mockClient,
		mapper: newMapper(),
	}
...
func TestProvider_GameBoxScore(t *testing.T) {
	// Setup mock data
	testGameId := "12345"
	mockBoxScore := mock_data2.CreateMockGameBoxScoreResponse(101, 111, 1, 4, "23.12.2022", "13.00")
	...
	// Setup mock client
	mockClient := &infobasket2.MockClient{
		BoxScoreFunc: func(gameId string) infobasket2.GameBoxScoreResponse {
			return mockBoxScore
		},
	}
...
func TestProvider_GamesByDate(t *testing.T) {
	...
	mockClient := &infobasket2.MockClient{
		ScheduledGamesFunc: func(compId int) []infobasket2.GameScheduleDto {
			if compId == 89960 {
				return []infobasket2.GameScheduleDto{
					mock_data2.CreateMockGameScheduleDto(1, searchingDate.Format(format), 1),
					mock_data2.CreateMockGameScheduleDto(2, firstWrongDate.Format(format), -1),
					mock_data2.CreateMockGameScheduleDto(3, secondWrongDate.Format(format), 0),
					mock_data2.CreateMockGameScheduleDto(4, searchingDate.Format(format), 99999),
				}
			} else if compId == 89962 {
				return []infobasket2.GameScheduleDto{
					mock_data2.CreateMockGameScheduleDto(5, searchingDate.Format(format), 1),
					mock_data2.CreateMockGameScheduleDto(6, searchingDate.Format(format), 0),
					mock_data2.CreateMockGameScheduleDto(7, secondWrongDate.Format(format), 1),
					mock_data2.CreateMockGameScheduleDto(8, firstWrongDate.Format(format), 1),
				}
			}
...
func TestProvider_GamesByTeam(t *testing.T) {
	mockClient := &infobasket2.MockClient{
		TeamGamesFunc: func(teamId string) infobasket2.TeamScheduleResponse {
			return infobasket2.TeamScheduleResponse{Games: []infobasket2.GameScheduleDto{
				mock_data2.CreateMockGameScheduleDto(1, "01.01.2025", 1),
				mock_data2.CreateMockGameScheduleDto(2, "02.01.2025", -1),
				mock_data2.CreateMockGameScheduleDto(3, "03.01.2025", 0),
				mock_data2.CreateMockGameScheduleDto(4, "04.01.2025", 99999),
			}}
		}}
