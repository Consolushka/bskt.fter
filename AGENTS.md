...
5. persistence сохраняет данные в БД, после чего обновляется watermark турнира.
...
- Изоляция и производительность (Транзакционный паттерн):
  ...
  - После завершения каждого теста транзакция откатывается (`Rollback()`) in `TearDownTest`, что гарантирует чистоту базы для следующего теста без необходимости пересоздания таблиц.
