...
- `app/cmd/create-tournament/main.go` — CLI-команда регистрации нового турнира (лига + привязка к провайдеру + даты сезона).

Текущая стратегия опроса:
- распределенная модель: каждый активный турнир обслуживается отдельной горутиной-воркером,
- динамическое обнаружение: список активных турниров периодически обновляется (интервал `SCHEDULER_REFRESH_INTERVAL_MINUTES`, по умолчанию 5), что позволяет запускать воркеры для новых турниров и останавливать для деактивированных без перезагрузки сервиса,
...
Базовый поток:
1. scheduler периодически обновляет список активных турниров, запуская независимые воркеры для новых турниров (с задержкой `stagger` между стартами) и останавливая воркеры для тех, что стали неактивными.
2. Каждый воркер циклично опрашивает данные. Для определения интервала опроса используется `interval_end` из последней успешной записи в таблице `tournament_poll_logs`.
3. orchestrator и processor запрашивают и обогащают данные через stats provider.
4. Выполняется двухфазная обработка игроков:
   - **Discovery**: Процессор проверяет наличие игрока в локальной БД.
   - **Ingestion**: Если игрока нет или данные неполные, запрашивается био у провайдера.
5. persistence сохраняет данные в БД.
6. В таблицу `tournament_poll_logs` записывается результат опроса (статус, время, количество сохраненных игр, ошибки).

Важно для лимитов API:
- Провайдеры реализованы так, чтобы разделять легкий листинг и тяжелое обогащение.
- Трансформеры в `infra` являются "чистыми" (Pure Functions) — они только мапят данные и не делают сетевых вызовов.
- **Нормализация данных**: Все процентные показатели (например, Field Goals Percentage) ОБЯЗАТЕЛЬНО должны быть приведены к диапазону `0.0 - 1.0` (где 1.0 соответствует 100%) на уровне трансформера, независимо от формата данных провайдера.

Отбор активных турниров:
- `TournamentsRepo.ListActive` не возвращает завершённые турниры (`end_at < now`) — чтобы не тратить лимиты внешних API на турниры без новых игр.
- `end_at IS NULL` означает «даты неизвестны»: такой турнир считается **активным** и продолжает опрашиваться (а не молча пропускается). Поэтому `start_at`/`end_at` в модели — указатели (`*time.Time`).

## 5a. Регистрация турнира

Создание турнира вынесено из SQL-сидов в явный use-case `service.TournamentRegistrar.Create` (точка входа — CLI `app/cmd/create-tournament`).

Поток:
1. Сборка провайдера через инжектируемую `ProviderFactory` (в проде — `providers.NewProvider`); она же валидирует, что для провайдера переданы нужные `externalId`/`params`.
2. Определение дат сезона по приоритету: **явные даты от пользователя → провайдер → предупреждение + `NULL`**.
3. `LeaguesRepo.FirstOrCreate` — гарантируем наличие лиги (идемпотентно, по ключу `(name, alias)`).
4. `TournamentsRepo.Create` — атомарно (одна транзакция) пишет турнир и его привязку к провайдеру.

Опциональная способность провайдера отдавать даты:
- порт `ports.TournamentPeriodProvider` (`GetTournamentPeriod(season)`) — **отдельный** интерфейс, не часть `StatsProvider`;
- реализуют только провайдеры, у которых даты есть в API (сейчас — `API_BASKETBALL` через `/leagues`);
- наличие способности проверяется приведением типа; не реализована → предупреждение, даты остаются `NULL`;
- провенанс (canonical/derived) пока не различается — заложить, когда появится первый «производный» провайдер (например, NBA через `min/max` по `/games`).
...
- **Ограничения (Constraints)**: Для колонок с процентами в таблице `game_team_player_stats` следует использовать `CHECK` констрейнты на уровне БД, чтобы гарантировать диапазон `[0, 1]`.
...
### 7.2 Тестирование адаптеров провайдеров
Все адаптеры стат-провайдеров должны быть покрыты unit-тестами с использованием моков клиентов (`MockClientInterface`). Тесты должны проверять:
- успешное получение данных и маппинг,
- фильтрацию игр по статусу,
- обработку ошибок API на разных этапах (листинг, детали),
- корректность работы с кешем (если применимо).

### 7.3 Общие требования к новым изменениям:
- happy-path,
- error-path,
- continue/skip-ветки,
...
6. **Отказоустойчивость**: Все долгоживущие горутины (воркеры шедулера) должны быть защищены через `defer logger.Recover(ctx)`.
7. Логи должны быть информативными и не шумными:
