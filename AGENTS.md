...
- Изоляция: база данных должна инициализироваться заново перед каждым тестом (`SetupTest`) для обеспечения чистоты данных.
- Автомиграция: в `SetupTest` необходимо передавать все модели, участвующие в тестах (включая связанные таблицы для JOIN).
