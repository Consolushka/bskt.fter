...
- `API_NBA` (полная поддержка статистики и био игроков)
- `INFOBASKET` (поддержка игр и статистики)
- `SPORTOTEKA` (поддержка игр и статистики)

Все активные провайдеры покрыты unit-тестами с использованием моков. Провайдер `CDN_NBA` присутствует в коде как заглушка.
...
- **Отказоустойчивость**: Каждый воркер защищен механизмом `Recover`. В случае критической ошибки (паники) во время обработки турнира, ошибка будет залогирована со стек-трейсом, а воркер продолжит работу в следующем цикле.
